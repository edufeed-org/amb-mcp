...
For grounded answering — the user wants an answer BUILT FROM the content, with citations, not a list of links — use search_passages (when available). Put only the topic in question. When the user restricts the sources ("nur Content von X", "only from X"), route the restriction into the scope parameters, never into question: a metadata publisher (most organisations: resolve_publisher(name) finds the exact spelling) goes into search as a quoted field filter, e.g. search:'publisher.name:"LEHRE LADEN"'; a Nostr signing account (resolve_author(name) → pubkey) goes into authors. Publisher names must be quoted inside the search value — an unquoted multi-word name silently matches nothing. Answer from the returned passages and cite each source (name + source_url/page).
